...
//endregion
    ...
    //endregion
    /**
     * Builds the BVH tree for the geometries in this object.
     * This method first flattens the list of geometries, then calls
     * the overloaded {@link #buildBvhTree(BoundingBox)} method using
     * the bounding box of the current geometries as the initial box.
     * After building the tree, it checks if there are any geometries
     * that could not be bound by a bounding box.
     */
    public void buildBvhTree() {
        flatten(); // Flatten the list of geometries
        setImperfectBoundingBox();
        if(getBoundingBox()==null)
            return;
        buildBvhTree(getBoundingBox()); // Recursively build the BVH tree starting from the bounding box
        checkForUnBoundability(); // Check if any geometries couldn't be bound by a bounding box
    }
     ...
     */
    public void buildBvhTree(@NotNull BoundingBox box) {
        boolean areThereAny = false;

        // Check which geometries can be contained within the bounding box
        for (var geometry : getContainerList()) {
            if (geometry instanceof Geometries innerGeometries)
                innerGeometries.setImperfectBoundingBox(); // Set imperfect bounding box for nested geometries

            if (geometry.getBoundingBox() == null)
                geometry.setBoundingBox(); // Ensure the geometry has a bounding box

            if (geometry.getBoundingBox() == null)
                continue; // Skip geometries that cannot be bounded

            if (box.contain(geometry.getBoundingBox()))
                areThereAny = true; // Mark that there are geometries within this bounding box
        }

        if (!areThereAny)
            return; // If no geometries can be bound within this box, return


        List<BoundingBox> subBoxes = box.divide(); // Divide the current bounding box into sub-boxes
        ...
        Geometries forThisBox = new Geometries(); // Container for geometries within this box
        ...
        for (var geometry : getContainerList()) {
            ...
            if (box.contain(geometry.getBoundingBox())) {
                forThisBox.add(geometry); // Add geometry to this box's container
            }
        }
        for(var g : forThisBox.getContainerList()){
            remove(g);
        }
        add(forThisBox);
    }
